...
private class SwitchSink<SourceType, S: ObservableConvertibleType, O: ObserverType>:
    ...
    func performMap(_: SourceType) throws -> S {
        rxAbstractMethod()
    }

    @inline(__always)
    private final func nextElementArrived(element: E) -> (Int, Observable<S.E>)? {
...
private final class SwitchSinkIter<SourceType, S: ObservableConvertibleType, O: ObserverType>:
    ObserverType,
    LockOwnerType,
...
private final class SwitchIdentitySink<S: ObservableConvertibleType, O: ObserverType>: SwitchSink<S, S, O> where O.E == S.E {
    override init(observer: O, cancel: Cancelable) {
        super.init(observer: observer, cancel: cancel)
    }
...
private final class MapSwitchSink<SourceType, S: ObservableConvertibleType, O: ObserverType>: SwitchSink<SourceType, S, O> where O.E == S.E {
...
private final class Switch<S: ObservableConvertibleType>: Producer<S.E> {
...
private final class FlatMapLatest<SourceType, S: ObservableConvertibleType>: Producer<S.E> {
    typealias Selector = (SourceType) throws -> S
