...
private final class ZipCollectionTypeSink<C: Collection, O: ObserverType>:
...
private final class ZipCollectionType<C: Collection, R>: Producer<R> where C.Iterator.Element: ObservableConvertibleType {
    ...
    init(sources: C, resultSelector: @escaping ResultSelector) {
        ...
        count = Int(Int64(self.sources.count))
    }
