...
//
    ...
    extension Reactive where Base: UITableView {
        ...
        public func setDataSource(_ dataSource: UITableViewDataSource)
            -> Disposable {
            return RxTableViewDataSourceProxy.installForwardDelegate(dataSource, retainDelegate: false, onProxyForObject: base)
        }
        ...
        public var itemInserted: ControlEvent<IndexPath> {
            let source = dataSource.methodInvoked(#selector(UITableViewDataSource.tableView(_:commit:forRowAt:)))
                .filter { a in
                    UITableViewCellEditingStyle(rawValue: (try castOrThrow(NSNumber.self, a[1])).intValue) == .insert
                }
                .map { a in
        ...
        public var itemDeleted: ControlEvent<IndexPath> {
            let source = dataSource.methodInvoked(#selector(UITableViewDataSource.tableView(_:commit:forRowAt:)))
                .filter { a in
                    UITableViewCellEditingStyle(rawValue: (try castOrThrow(NSNumber.self, a[1])).intValue) == .delete
                }
                .map { a in
                    try castOrThrow(IndexPath.self, a[2])
                }
        ...
        /**
         ...
         or any other data source conforming to `SectionedViewDataSourceType` protocol.
          ...
          ```
         */
        public func modelSelected<T>(_: T.Type) -> ControlEvent<T> {
        ...
        /**
         ...
         ```
         */
        public func modelDeselected<T>(_: T.Type) -> ControlEvent<T> {
        ...
        /**
         ...
         ```
         */
        public func modelDeleted<T>(_: T.Type) -> ControlEvent<T> {
